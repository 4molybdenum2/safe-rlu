...
fn read_write(set: ConcurrentBTreeSet<usize>, config: BenchmarkConfig) -> BenchmarkResult {
            ...
            loop {
                ...
                if _rnd.gen::<f64>() < config.write_ratio {
                    ...
                    if _rnd.gen::<f64>() < config.insert_ratio {
                        ...
                        set.delete(num);
                        results.delete += 1;
                        results.delete_times += curr.elapsed().as_nanos();
                    } else {
                        let curr = Instant::now();
                        set.insert(num);
                        results.insert += 1;
                        results.insert_times += curr.elapsed().as_nanos();
                    }
                } else {
                    let curr = Instant::now();
                    // read operation
    ...
    threads
        .into_iter()
        .map(|t| t.join().unwrap())
        .fold(BenchmarkResult::default(), |mut x, res| {
            x.ops += res.ops;
            ...
            x.insert += res.insert;
            x.insert_times += res.insert_times;

            x.delete += res.delete;
            x.delete_times += res.delete_times;
...
fn benchmark() {
    ...
    for wr in &[0.02, 0.2, 0.4] {
        for i in 1..=8 {
            ...
            let ops: Vec<_> = (0..3)
                .map(|_| {
                    let set = ConcurrentBTreeSet::new();
                    let mut _rnd = SmallRng::from_seed([0; 16]);
            ...
            let avg: f64 =
                (ops.iter().map(|res| res.ops).sum::<usize>() as f64) / (ops.len() as f64);
